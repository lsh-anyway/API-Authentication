// Will work for linux for windows we are going to use cross-env in package.json
...
describe("Users controller", () => {
  ...
  describe("secret", () => {
    it("should return source when called", () => {
  ...
  describe("signin", () => {
    it("should be return token when signIn called", () => {
      sandbox.spy(res, "json");
  ...
  describe("signUp", () => {
    it("should return 403 if the user is already save in the database.", () => {
      sandbox.spy(res, "json");
      sandbox.spy(res, "status");
      sandbox
        .stub(User, "findOne")
    ...
    it("should return 200 if user is not in database and it was saved", () => {
      sandbox.spy(res, "json");
      sandbox.spy(res, "status");
    ...
    it("should return 200 if user is not in database using callback done", done => {
      sandbox.spy(res, "json");
      sandbox.spy(res, "status");
      sandbox.stub(User, "findOne").returns(Promise.resolve(false));
      sandbox
      ...
      /*
        ...
        as we know the userController.signUp returns a promise
        and expect result spy to have been called as expect
       */
